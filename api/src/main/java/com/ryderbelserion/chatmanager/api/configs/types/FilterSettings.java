...
public class FilterSettings implements SettingsHolder {
    ...
    @Comment("If a player executes one of these commands with a swear/advertisement in it, the command will be blocked.")
    public static final Property<List<String>> BLACKLISTED_COMMANDS = newListProperty("settings.commands.blacklist", List.of(
            "plugins",
            "pl"
    ));

    @Comment("If a player executes one of these commands with a swear/advertisement in it, the command will not be blocked.")
