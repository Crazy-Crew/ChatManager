		...
		if (cmd.getName().equalsIgnoreCase("ChatManager")) {
			if (args.length == 0) {
				sender.sendMessage(Methods.color("&7This server is using the plugin &cChatManager "
						+ "&7version " + main.getDescription().getVersion() + " by &cH1DD3NxN1NJA."));
			...
			if (args[0].equalsIgnoreCase("debug")) {
				...
				if (args.length == 1) {
					sender.sendMessage("");
					sender.sendMessage(Methods.color(" &3Chat Manager Debug Help Menu &f(v" + main.getDescription().getVersion() + ")"));
					sender.sendMessage("");
				...
				if (args[0].equalsIgnoreCase("help")) {
					if (args.length == 1) {
						if (Version.getCurrentVersion().isNewer(Version.v1_8_R2) && Version.getCurrentVersion().isOlder(Version.v1_17_R1)) {
							...
							JSONMessage.create(" &3Chat Manager &f(v" + main.getDescription().getVersion() + ")").send(player);
							JSONMessage.create("").send(player);
						...
						player.sendMessage("");
						player.sendMessage(Methods.color(" &3Chat Manager &f(v" + main.getDescription().getVersion() + ")"));
						player.sendMessage("");
